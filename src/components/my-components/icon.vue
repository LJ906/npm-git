<style scoped>
  .iconStyle{text-align: center;font-size:14px;cursor:pointer}
  .active{color:yellow}
  .iconModalStyle{height:800px;width:1500px;background:#fff;position:fixed;top:100px;border-radius:7px;box-shadow: 4px 4px 10px #ddd;left:50%;margin-left:-700px;z-index:9999;}
</style>
<template>
  <div class="iconModalStyle">
    <h3 style="height:50px;border-bottom:1px solid #ddd;line-height:50px;padding-left:10px;">请选择图标</h3>
    <Row style="height:680px;overflow-y: scroll;border-bottom:1px solid #ddd;">
      <Col span="3"  v-for="(icon,index) in iconType" :key="index">
        <div :class="{'iconStyle':iconStyle,'active':index==current}"  @click="checkIcon(index)">
            <p><Icon :type=icon></Icon></p>
            <p>{{icon}}</p>
        </div>
      </Col>
    </Row>
    <p style="text-align:right;padding:10px;">
      <Button type="primary" @click="iconSure">确定</Button>
    </p>
  </div>
</template>
...
<script>
  export default {
    ...
    methods:{
      checkIcon:function(idx){
        this.current = idx
      },
      iconSure:function(){
        this.checkClass = this.iconType[this.current];
        this.$emit('iconSure',this.checkClass);
      }
